...
import { RiShoppingCart2Line, MdSearch, BsArrowRightShort, MdKeyboardArrowRight, CgProfile, IoIosHeart } from 'react-icons/all';
import { IoLogOutOutline, IoArrowDownCircleOutline } from 'react-icons/io5';
...
const Nav = ({ history }) => {
    ...
    return (
        <nav ref={Nav} className={`nav ${nav ? 'active' : ''}`} >
            <div className="logo"><Link to=''>AVON</Link></div>
            ...
            <div className="rightComp" ref={rightItems}>
                ...
                <Link to='/likes' activeClassName='activlink'>
                    <IoIosHeart className='iconHeart' size='25' />
                </Link>}
                {userInfo && userInfo.isAdmin && (
                    <Menu>
                        <MenuButton as={Button} rightIcon={<IoArrowDownCircleOutline />}>
                            Admin
                        </MenuButton>
                        <MenuList>
                            <MenuItem>
